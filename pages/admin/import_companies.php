<?php
/**
 * pages/admin/import_companies.php
 * Import Excel → insère directement dans la table companies
 */
    ...
    for ($i = 1; $i <= 10; $i++) {
        ...
        foreach ($xml->sheetData->row as $row) {
            ...
            foreach ($row->c as $cell) {
                ...
                $rowData[] = trim(preg_replace('/[\x{00A0}\x{200B}]/u', ' ', $val));
            }
...
// ── Extraire entreprises uniques (colonne C = index 2) ────────
function extractCompanies(array $rows): array {
    $seen = [];
    foreach ($rows as $row) {
        $name = trim($row[2] ?? '');
        if (!$name || strtolower($name) === 'entreprise') continue;
        $key = strtolower($name);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['excel_file'])) {
    ...
    if ($ext !== 'xlsx') {
        ...
        $companies = extractCompanies($rows);

        if (empty($companies)) {
            $errors[] = "Aucune entreprise trouvée dans le fichier.";
        } else {
            // Aperçu : 15 premières avec statut existe/nouveau
            foreach (array_slice($companies, 0, 15) as $name) {
                $exists = dbGet('SELECT id FROM companies WHERE LOWER(name) = LOWER(?)', [$name]);
                $preview[] = ['name' => $name, 'exists' => (bool)$exists];
            }
            $_SESSION['import_companies'] = $companies;
        }
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['confirm_import'])) {
    $companies = $_SESSION['import_companies'] ?? [];
    $inserted  = 0;
    $skipped   = 0;
    ...
    foreach ($companies as $name) {
        ...
        if ($exists) {
            ...
            dbRun(
                'INSERT INTO companies (name, sector, location, description, contact_email, website) VALUES (?,?,?,?,?,?)',
                [$name, '', '', '', '', '']
            );
            $inserted++;
        }
    ...
    unset($_SESSION['import_companies']);
    $result = compact('inserted', 'skipped');
}

renderHead('Import entreprises'); renderHeader();
?>

<section class="page-hero compact">
  <div class="container">
    <span class="eyebrow">Administration</span>
    <h1>Importer des entreprises depuis Excel</h1>
    <p>Les entreprises du fichier seront ajoutées directement dans la liste des entreprises partenaires.</p>
  </div>
</section>
  ...
  <div class="container" style="max-width:760px">
    ...
    <div class="glass-card" style="border:2px solid var(--success);background:var(--success-bg);margin-bottom:2rem;">
      ...
      <div class="stats-row" style="margin:0 0 1.5rem">
        <div class="stat-card"><strong><?= $result['inserted'] ?></strong><span>Entreprises ajoutées</span></div>
        <div class="stat-card"><strong><?= $result['skipped'] ?></strong><span>Doublons ignorés</span></div>
      </div>
      <div style="display:flex;gap:1rem;flex-wrap:wrap;">
        <a href="/entreprises" class="btn btn-primary">Voir les entreprises</a>
        <a href="/admin/import-companies" class="btn btn-outline">Importer un autre fichier</a>
      </div>
    </div>
    ...
    <div class="glass-card" style="margin-bottom:2rem;">
      <h3 style="margin-bottom:.5rem;">Aperçu — <?= count($_SESSION['import_companies'] ?? []) ?> entreprises détectées</h3>
      ...
      <?php $total = count($_SESSION['import_companies'] ?? []); ?>
      <?php if ($total > 15): ?>
        <p style="font-size:.85rem;color:var(--text-2);margin-top:.75rem;">
          … et <?= $total - 15 ?> entreprises supplémentaires.
        </p>
      <?php endif; ?>
      <form method="POST" action="/admin/import-companies" style="margin-top:1.5rem;display:flex;gap:1rem;flex-wrap:wrap;">
        <button type="submit" name="confirm_import" value="1" class="btn btn-primary">
          ✅ Confirmer l'import (<?= $total ?> entreprises)
        </button>
        <a href="/admin/import-companies" class="btn btn-outline">Annuler</a>
      </form>
    </div>
    ...
    <div class="glass-card">
      ...
      <form method="POST" action="/admin/import-companies" enctype="multipart/form-data" class="form-grid">
        <div>
    ...
    <div style="margin-top:1.5rem;">
      <a href="/entreprises" class="btn btn-outline">← Retour aux entreprises</a>
    </div>
